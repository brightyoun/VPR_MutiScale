...
class Session:
    ...
    def inf_batch(self, name, batch):
        ...
        L = batch['L'].cuda()
        print("아이엔에프배치 : ", O.shape, B.shape, type(O))
        O, B = Variable(O, requires_grad=False), Variable(B, requires_grad=False)
        print("아이엔에프배치22 : ", O.shape, B.shape, type(O), L.shape, L)

        '''입력이 O고 모델은 self.net, 모델의 prediction은 derain'''
        derain = self.net(O)
...
def run_train_val(ckp_name_net='latest_net'):
    ...
    while sess.step < settings.total_step:
        ...
        for i, (batchs, labels, _) in enumerate(sess.get_dataloader('train')):
            print('야야 : ', i, batchs, labels, _)
        ...
        loss_all = 0
        num_all = 0        
        if sess.step % (settings.one_epoch * 16) == 0:            
